...
{
    if (0 == gbmcmc_data->mcmc_step % 1000) {
        fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler start, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step);
    }
    clock_t start = clock();
...
#pragma omp parallel num_threads(flags->threads)
    {
        ...
        if (0 == gbmcmc_data->mcmc_step % 1000) {
            fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp parallel start threadId %i, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, threadID);
        }
#pragma omp barrier
        ...
        for(int ic=threadID; ic<NC; ic+=numThreads)
        {
		    ...
		    if (0 == gbmcmc_data->mcmc_step % 1000) {
        	    fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp barrier start threadId %i looping over threadId %i likelihood start, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, threadID, ic);
			}
            //update model likelihood using new residual
            model_ptr->logL = gaussian_log_likelihood(data, model_ptr);
		    if (0 == gbmcmc_data->mcmc_step % 1000) {
        	    fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp barrier start threadId %i looping over threadId %i likelihood end, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, threadID, ic);
			}
            //sync up model and trial pointers
            copy_model(model_ptr,trial_ptr);

		    if (0 == gbmcmc_data->mcmc_step % 1000) {
        	    fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp barrier start threadId %i looping over threadId %i galactic_binary_rjmcmc start, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, threadID, ic);
			}
            ...
            if (0 == gbmcmc_data->mcmc_step % 1000) {
	            fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp barrier start threadId %i looping over threadId %i  galactic_binary_rjmcmc end and galactic_binary_fisher start, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, threadID, ic);
            }
            ...
            if (0 == gbmcmc_data->mcmc_step % 1000) {
	            fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp barrier start threadId %i looping over threadId %i  galactic_binary_fisher end, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, threadID, ic);
			}
...
#pragma omp barrier
    ...
    if (0 == gbmcmc_data->mcmc_step % 1000) {
	    fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler pragma omp barrier end, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step);
	}
    ...
    if (0 == gbmcmc_data->mcmc_step % 1000) {
	    fprintf(stdout,"GBMCMC Process %i on step %i: update_gbmcmc_sampler end, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step);
    }
    return 1;
}
...
{
    if (0 == gbmcmc_data->mcmc_step % 1000) {
	    fprintf(stdout,"GBMCMC Process %i on step %i: exchange_gbmcmc_source_params start, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step);
	}
    ...
    /* update likelihoods */
    if (0 == gbmcmc_data->mcmc_step % 1000) {
	    fprintf(stdout,"GBMCMC Process %i on step %i: exchange_gbmcmc_source_params likelihoods start all chains, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step);
	}
    for(int ic=0; ic<chain->NC; ic++)
    {
        ...
        {
            if (0 == gbmcmc_data->mcmc_step % 1000) {
	            fprintf(stdout,"GBMCMC Process %i on step %i: exchange_gbmcmc_source_params likelihoods start chain %i, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step, ic);
			}
    ...
    if (0 == gbmcmc_data->mcmc_step % 1000) {
	    fprintf(stdout,"GBMCMC Process %i on step %i: exchange_gbmcmc_source_params end, \n", gbmcmc_data->procID, gbmcmc_data->mcmc_step);
	}
}
